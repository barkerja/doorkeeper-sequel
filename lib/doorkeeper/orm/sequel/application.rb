module Doorkeeper
  # Application Model for Doorkeeper Sequel ORM
  class Application < Sequel::Model(:oauth_applications)
    include Doorkeeper::Models::SequelCompat
    include Doorkeeper::OAuth::Helpers

    one_to_many :authorized_tokens, conditions: { revoked_at: nil },
                                    class: 'Doorkeeper::AccessToken'

    def self.authorized_for(resource_owner)
      AccessToken.where(resource_owner_id: resource_owner.id, revoked_at: nil)
                 .map(&:application)
    end
    ...
    plugin :association_dependencies, access_grants: :destroy,
    ...
    def validate
      ...
      RedirectUriValidator.new(attributes: values).validate_each(
